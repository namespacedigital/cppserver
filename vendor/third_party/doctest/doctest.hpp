...
#define DOCTEST_INTERNAL_ERROR(msg)                                                                \
    ...
    void String::setSize(size_type sz) noexcept {
        if (isOnStack()) { buf[sz] = '\0'; setLast(last - sz); } else { data.ptr[sz] = '\0'; data.size = sz; }
    }
    ...
    String::size_type String::find(char ch, size_type pos) const {
        ...
        for (; it < end && *it != ch; it++);
        if (it < end) { return static_cast<size_type>(it - begin); } else { return npos; }
    }
    ...
    String::size_type String::rfind(char ch, size_type pos) const {
        ...
        if (it >= begin) { return static_cast<size_type>(it - begin); } else { return npos; }
    }
...
#define DOCTEST_GENERATE_ASSERT_TYPE_CASES(assert_type) \
    ...
    DOCTEST_DEFINE_INTERFACE(IContextScope)
        ...
        void filldata<const void*>::fill(std::ostream* stream, const void* in) {
            if (in) { *stream << in; } else { *stream << "nullptr"; }
        }
